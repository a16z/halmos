...
import halmos.parser
from halmos.parser import ConfigFileProvider, ConfigParser
...
def mock_config_parser(config_file_provider):
    halmos.parser.config_parser = ConfigParser(
        config_file_provider=config_file_provider
    )
    return halmos.parser.get_config_parser()
...
def test_config_extend_does_not_modify_original():
    ...
    # the base value can be accessed from the new config
    assert new_config.verbose == 1234
